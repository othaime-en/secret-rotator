...
class PassphraseManager:
    ...
    def prompt_interactive(
        ...
        while True:
            ...
            if len(passphrase) < min_length:
                print(
                    f"⚠️  WARNING: Passphrase is only {len(passphrase)} characters "
    ...
    def print_help_message(self, is_docker: Optional[bool] = None):
        ...
        if is_docker:
            ...
            print(
                "   docker exec secret-rotator chmod 600 /app/data/.backup-passphrase",
                file=sys.stderr,
            )
        else:
            print("   mkdir -p ~/.config/secret-rotator", file=sys.stderr)
            print(
                "   echo 'your-passphrase' > ~/.config/secret-rotator/.backup-passphrase",
                file=sys.stderr,
            )
        ...
        print(
            "   secret-rotator-backup create-encrypted --passphrase-file /path/to/file",
            file=sys.stderr,
        )
        print()
    ...
    def create_passphrase_file(
        self, file_path: str, passphrase: Optional[str] = None, interactive: bool = True
    ) -> bool:
        """
        ...
        if not passphrase and interactive:
            try:
                passphrase = self.prompt_interactive(
                    purpose="storage in file", require_confirmation=True
                )
